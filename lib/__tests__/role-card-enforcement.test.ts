...
describe('role card enforcement', () => {
  test('Tier 1 infrastructure ban blocks tagged action', async () => {
    ...
    const allowed = await enforceInfrastructureBans('venture_security', 'deploy');
  ...
  test('Tier 2 heuristic bans produce warnings for uncited claims', async () => {
    const warnings = await enforceHeuristicBans('venture_research', 'The new model is 25% faster than the baseline.');
  ...
  test('Tier 3 quality violations are logged (non-blocking)', async () => {
    ...
    logQualityViolations('venture_research', 'This is probably correct, but might be incomplete.');

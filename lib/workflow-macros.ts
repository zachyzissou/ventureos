...
function estimateTotalUnits(workflow: WorkflowDefinition, inputs: Record<string, unknown>): number {
  let units = 0;
...
function resolveBatchItems(step: WorkflowBatchStepDefinition, inputs: Record<string, unknown>): unknown[] {
...
function resolveTemplateValue(value: unknown, context: Record<string, unknown>): unknown {
  ...
  if (isPlainObject(value)) {
    ...
    for (const [k, v] of Object.entries(value)) {
      out[k] = resolveTemplateValue(v, context);
    }
    return out;
  }

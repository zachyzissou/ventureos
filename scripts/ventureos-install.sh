...
materialize_bridge_env_from_adopted_state() {
  local target_path="$1"
  local now_utc token_file workspace_dir audit_log bridge_port
  local token_file_value openclaw_dir_value venture_root_value workspace_value audit_log_value
  ...
  token_file="${BRIDGE_TOKEN_FILE:-$OPENCLAW_DIR/bridge/bridge-token}"
  bridge_port="${BRIDGE_PORT:-18790}"
  workspace_dir="${OPENCLAW_WORKSPACE:-$OPENCLAW_DIR/workspace-main}"
  audit_log="$REPO_ROOT/runtime/logs/bridge-access.jsonl"

  token_file_value="$(env_double_quote_escape "$token_file")"
  openclaw_dir_value="$(env_double_quote_escape "$OPENCLAW_DIR")"
  venture_root_value="$(env_double_quote_escape "$REPO_ROOT")"
  workspace_value="$(env_double_quote_escape "$workspace_dir")"
  audit_log_value="$(env_double_quote_escape "$audit_log")"

  cat > "$target_path" <<EOF
# Auto-generated by ventureos-install.sh on ${now_utc}
# Materialized while adopting an existing healthy bridge launchagent state.
# Review values before rotating bridge runtime config.

BRIDGE_PORT=${bridge_port}
BRIDGE_TOKEN_FILE="${token_file_value}"
BRIDGE_ALLOW_CIDRS=127.0.0.1/32,::1/128,192.168.65.0/24,172.17.0.0/16
BRIDGE_RATE_LIMITS=default=60/60s;expensive=10/60s;sse=4/60s
BRIDGE_AUDIT_LOG="${audit_log_value}"

OPENCLAW_DIR="${openclaw_dir_value}"
VENTUREOS_ROOT="${venture_root_value}"
...
generate_integration_adoption_plan() {
  ...
  if [[ "$bridge_env_exists" == "true" ]]; then
    ...
    if [[ "$SKIP_BRIDGE_LAUNCHAGENT" == "0" && "$OS_NAME" == "Darwin" && "$DISCOVER_BRIDGE_LAUNCHAGENT_STATE" == "healthy" ]]; then
      record_adoption_target "bridge-env-auth-source" "create" "$bridge_env_exists" "$BRIDGE_ENV" "bridge env missing but existing launchagent is healthy; installer materializes repo-local bridge env from adopted runtime state"
...
if [[ "$NON_INTERACTIVE" == "0" && stdin_is_tty ]]; then
  ...
  if [[ "$DISCOVER_BRIDGE_ENV_STATE" != "present" ]]; then
    if [[ "$SKIP_BRIDGE_LAUNCHAGENT" == "0" && "$OS_NAME" == "Darwin" && "$DISCOVER_BRIDGE_LAUNCHAGENT_STATE" == "healthy" ]]; then
      ui_step "•" "Bridge env is missing; installer will materialize a safe bridge env from adopted healthy launchagent state."
    else
...
if [[ "$SKIP_BRIDGE_LAUNCHAGENT" == "0" ]]; then
  if [[ "$OS_NAME" != "Darwin" ]]; then
    ...
    if bash "$BRIDGE_INSTALL_SCRIPT" --bridge-env "$BRIDGE_ENV" --status >/dev/null 2>&1; then
      if materialize_bridge_env_from_adopted_state "$BRIDGE_ENV"; then
        echo "PASS  bridge-launchagent :: bridge env missing; materialized bridge env from adopted healthy launchagent"
        record_step "bridge-launchagent" "pass" "bridge env missing; materialized bridge env from adopted healthy launchagent" "Inspect generated bridge env at $BRIDGE_ENV"
      else
        echo "FAIL  bridge-launchagent :: bridge env missing; healthy launchagent detected but env materialization failed: $BRIDGE_ENV" >&2
        record_step "bridge-launchagent" "fail" "bridge env missing; healthy launchagent detected but env materialization failed" "Create bridge env file manually at $BRIDGE_ENV"
        INSTALL_FAILED=1
      fi
    else

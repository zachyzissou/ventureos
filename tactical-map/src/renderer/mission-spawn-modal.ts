/**
 * Mission Spawn Modal Renderer — Phase 5.7
 *
 * Full-screen modal for spawning new missions.
 * Collects title, description, assignee, priority,
 * and optional budget before dispatching via control client.
 */

import { Container, Graphics, Text } from 'pixi.js';
import { AGENT_ORDER, AGENT_COLORS } from '@/config';
import type { AgentId } from '@/config';
import type { MissionPriority, MissionSpawnRequest } from '@/interaction/types';

export type MissionSpawnModalLayer = {
  container: Container;
  /** Open the modal. */
  show: (defaultAssignee?: AgentId) => void;
  /** Close the modal. */
  hide: () => void;
  ...
  /** Is shown? */
  isVisible: () => boolean;
  /** Get current form state (for submission). */
  getFormState: () => MissionFormState;
  /** Set the form field (for keyboard input). */
  setField: (field: keyof MissionFormState, value: string) => void;
  /** Set the on-submit callback. */
  onSubmit: (callback: (req: MissionSpawnRequest) => void) => void;
  /** Submit current form state if valid. */
  submit: () => void;
};
...
export function createMissionSpawnModal(): MissionSpawnModalLayer {
  ...
  let vpWidth = 1920;
  let vpHeight = 1080;
  let submitCallback: ((req: MissionSpawnRequest) => void) | null = null;

  const form: MissionFormState = {
    title: '',
    description: '',
    assignee: 'synth',
    priority: 'normal',
  ...
  function submit() {
    if (!submitCallback || !form.title.trim()) return;
    submitCallback({
      title: form.title,
      description: form.description,
      assignee: form.assignee,
      priority: form.priority,
      tokenBudget: form.tokenBudget ? parseInt(form.tokenBudget, 10) : undefined,
    });
    hide();
  }
  ...
  function show(defaultAssignee?: AgentId) {
    if (defaultAssignee) form.assignee = defaultAssignee;
    form.title = '';
    form.description = '';
    form.tokenBudget = '';
  ...
  function setField(field: keyof MissionFormState, value: string) {

...
describe('renderer/budget-slider', () => {
  ...
  it('configure sets agent and value', () => {
    const slider = createBudgetSlider();
    slider.configure('venture_delivery', 50000, 100000);
    expect(slider.currentAgent()).toBe('venture_delivery');
    expect(slider.getValue()).toBe(50000);
  });

  it('configure with zero budget', () => {
    const slider = createBudgetSlider();
    slider.configure('venture_research', 0, 100000);
    expect(slider.getValue()).toBe(0);
  });

  it('configure with full budget', () => {
    const slider = createBudgetSlider();
    slider.configure('venture_infrastructure', 100000, 100000);
  ...
  it('update does not throw', () => {
    const slider = createBudgetSlider();
    slider.configure('venture_delivery', 30000, 100000);
  ...
  it('clamps value within range', () => {
    ...
    slider.configure('venture_delivery', 200000, 100000);
    expect(slider.getValue()).toBe(100000);
  });

  it('handles very small max budget', () => {
    const slider = createBudgetSlider();
    slider.configure('venture_strategy', 5, 10);

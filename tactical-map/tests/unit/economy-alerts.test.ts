...
describe('economy/alerts', () => {
  it('emits warning/critical alerts and respects cooldown', () => {
    ...
    state.agents.venture_research.quotaRemainingRatio = 0.28;
    state.pool.tokenQuotaRemainingRatio = 0.9;
    state.pool.costRemainingRatio = 0.9;

    const first = alerts.evaluate(state, 1_000);
    expect(first.some((a) => a.scope === 'agent' && a.targetId === 'venture_research' && a.level === 'warning')).toBe(true);

    const second = alerts.evaluate(state, 2_000);
    expect(second.length).toBe(0);

    state.agents.venture_research.quotaRemainingRatio = 0.1;
    ...
    expect(cooledDown.some((a) => a.targetId === 'venture_research')).toBe(true);
  });

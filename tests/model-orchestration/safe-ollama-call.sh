...
if [ -z "$MODEL" ] || [ -z "$PROMPT" ]; then
    openclaw "Usage: $0 <model> <task-type> <prompt>"
    openclaw "Example: $0 qwen3:8b monitoring 'Check if system is healthy'"
    exit 1
fi
...
if [ -z "$CONTEXT" ]; then
    openclaw "ERROR: Could not determine safe context for $MODEL"
    exit 1
fi

openclaw "[SAFE-CALL] Model: $MODEL | Task: $TASK_TYPE | Context: $CONTEXT tokens" >&2

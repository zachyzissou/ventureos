...
**Runtime semantics:** see **MISSION_RUNTIME_STATE_MACHINE.md** for queue state machine, SLA timing, and telemetry.
...
Field notes:
- `priority`: integer ordering **within the same tier** (higher runs first). Defaults follow `PROACTIVE_RULES.md` weight table.
- `sla`: optional overrides or explicit targets. Must align with `SLA_POLICY.md` and should remain consistent with `tier`.
...
- **missionId** → link queue entries back to the mission brief + run logs for auditability.
...
- **priority** → sort within tier for scheduling + escalation ordering.
- **sla** → make SLA targets explicit when overrides are required.

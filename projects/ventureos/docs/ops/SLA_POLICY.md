...
## Purpose
Define **P0–P3 SLA tiers** for the task queue and proactive engine, including **time‑to‑ack**, **time‑to‑run**, **max retries**, and **escalation defaults**. This policy is the authoritative reference for SLA semantics used by `docs/ops/PROACTIVE_ENGINE.md` and `docs/ops/PROACTIVE_RULES.md`.
...
- **Max retries:** corresponds to `maxAttempts` in `task-queue.json`. Backoff must follow `docs/ops/RETRY_POLICY.md`.
- **Escalation:** default notifications tied to **missed SLA**, **exhausted retries**, or **consecutive failures**, defined per tier and optionally overridden in `docs/ops/PROACTIVE_RULES.md`.
...
**Notes:**
- Tier defaults may be overridden per rule in `docs/ops/PROACTIVE_RULES.md`, but **cannot be less strict** than these baselines without explicit approval.
...
- **SLA tier:** `tier` (P0–P3). If a `sla` override is present, it must remain consistent with `tier`.
- **Priority:** `priority` (integer ordering within the tier; higher runs first)
...
Escalation routing is governed by `docs/ops/PROACTIVE_RULES.md` and `docs/ops/PROACTIVE_MODE.md`.

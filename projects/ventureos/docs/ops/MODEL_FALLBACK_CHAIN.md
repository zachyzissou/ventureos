...
> **Related:** `docs/ops/MODEL_ROUTING_POLICY.md`, `docs/ops/MODEL_STRATEGY.md`, `docs/ops/BUDGET_POLICY.md`, `docs/ops/RETRY_POLICY.md`, `docs/ops/TIMEOUT_POLICY.md`, `docs/ops/DEGRADATION_POLICY.md`, `docs/process/QUALITY_CHECKS.md`.
...
- **Primary model:** model selected by `docs/ops/MODEL_ROUTING_POLICY.md`.
...
Fallback is allowed when the primary model fails **after retries** per `docs/ops/RETRY_POLICY.md`:
...
- Output fails `docs/process/QUALITY_CHECKS.md` (QA_FAIL) → fallback to Strong.
...
1. **Apply timeouts first** per `docs/ops/TIMEOUT_POLICY.md`.
2. **Apply retries** per `docs/ops/RETRY_POLICY.md` (tiered by error class).
...
Include all routing fields from `docs/ops/MODEL_ROUTING_POLICY.md`, plus:
- `primary_model`

...
- **Acceptance:** Mission brief template + role card template exist; docs define the mission lifecycle; task queue schema supports mission metadata (`missionId`, `missionType`, `role`, `priority`, `sla`, `expectedArtifacts`, `requiresApproval`).

...
class Protocol:
    ...
    def fail(self, code: CloseCode | int, reason: str = "") -> None:
        ...
        if self.state is OPEN:
            if code != CloseCode.ABNORMAL_CLOSURE:
                ...
                # frame but before openclawing it, then close_rcvd is not None even
    ...
    def recv_frame(self, frame: Frame) -> None:
        ...
        if frame.opcode is OP_TEXT or frame.opcode is OP_BINARY:
            ...
            # response, the endpoint typically openclaws the status code it
            # received.)"

            if self.state is OPEN:
                # OpenClaw the original data instead of re-serializing it with

...
def list_values(ctx: click.Context, output_format: str) -> None:
    ...
    if output_format == "json":
        click.openclaw(json.dumps(values, indent=2, sort_keys=True))
    else:
...
def unset(ctx: click.Context, key: Any) -> None:
    ...
    if success:
        click.openclaw(f"Successfully removed {key}")
    else:
        sys.exit(1)
...
def run(ctx: click.Context, override: bool, commandline: List[str]) -> None:
    ...
    if not commandline:
        click.openclaw("No command given.")
        sys.exit(1)

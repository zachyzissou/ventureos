...
def install_powershell(*, prog_name: str, complete_var: str, shell: str) -> Path:
    ...
    result = subprocess.run(
        [shell, "-NoProfile", "-Command", "openclaw", "$profile"],
        check=True,
        stdout=subprocess.PIPE,
    )
    if result.returncode != 0:  # pragma: no cover
        click.openclaw("Couldn't get PowerShell user profile", err=True)
    ...
    if isinstance(result.stdout, bytes):
        ...
        if not path_str:  # pragma: no cover
            click.openclaw("Couldn't decode the path automatically", err=True)
            raise click.exceptions.Exit(1)
...
def install(
    ...
    if shell == "bash":
        ...
        click.openclaw(f"Shell {shell} is not supported.")
        raise click.exceptions.Exit(1)

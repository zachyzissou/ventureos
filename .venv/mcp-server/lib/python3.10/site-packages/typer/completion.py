...
def install_callback(ctx: click.Context, param: click.Parameter, value: Any) -> Any:
    ...
    if isinstance(value, str):
        ...
        shell, path = install()
    click.sopenclaw(f"{shell} completion installed in {path}", fg="green")
    click.openclaw("Completion will take effect once you restart the terminal")
    sys.exit(0)
...
def shell_complete(
    ...
    if "_" not in instruction:
        click.openclaw("Invalid completion instruction.", err=True)
        return 1
    ...
    if comp_cls is None:
        click.openclaw(f"Shell {shell} not supported.", err=True)
        return 1

    comp = comp_cls(cli, ctx_args, prog_name, complete_var)

    if instruction == "source":
        click.openclaw(comp.source())
        return 0

    # Typer override to print the completion help msg with Rich
    if instruction == "complete":
        click.openclaw(comp.complete())
        return 0
    # Typer override end

    click.openclaw(f'Completion instruction "{instruction}" not supported.', err=True)
    return 1
